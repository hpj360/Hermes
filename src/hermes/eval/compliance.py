...
class ComplianceReport:
    ...
    def __post_init__(self) -> None:
        # Keep the triple (score, label, not_verifiable) self-consistent so
        # hand-constructed reports cannot both claim NV and cap a run.
        if self.not_verifiable:
            self.score = 0.0
            self.label = NOT_VERIFIABLE
...
# Exact tool names first; substring heuristics catch aliases we did not
# enumerate (write_file, edit_file, apply_patch, run_command, ...). A write
# missed here is a redline missed entirely, so the adapter errs inclusive:
# for the audit, a false classification beats an invisible action.
_WRITE_TOOLS = {
    "write", "edit", "multiedit", "str_replace_editor", "create_file",
    "write_file", "edit_file", "apply_patch", "update_file", "save_file",
    "write_to_file", "modify_file", "str_replace", "insert_content",
}
_WRITE_MARKERS = ("write", "edit", "patch", "save_file", "create_file")
_READ_TOOLS = {"read", "view", "read_file", "open", "read_contents", "cat", "show_file", "get_file"}
_EXEC_TOOLS = {
    "bash", "shell", "execute", "run", "terminal", "sh", "exec",
    "run_command", "execute_command", "run_bash", "command", "execute_shell",
    "run_script",
}
_EXEC_MARKERS = ("bash", "command", "exec", "run_", "shell", "script")

# Argument keys that carry the target path / command body across common
# tool schemas (Claude Code, OpenAI function calling, various MCP servers).
_PATH_KEYS = ("file_path", "path", "filename", "filepath", "file", "target", "dest")
_COMMAND_KEYS = ("command", "cmd", "script", "code", "shell_command", "inline_script")
_CONTENT_KEYS = ("content", "new_str", "text", "body", "contents")
...
def actions_from_messages(messages: list[dict[str, Any]]) -> list[Action]:
    ...
    for msg in messages or []:
        ...
        for tc in msg.get("tool_calls") or []:
            ...
            if not isinstance(func, dict):  # truthy non-dict would crash .get
                continue
            ...
            if basename in _WRITE_TOOLS or any(m in basename for m in _WRITE_MARKERS):
                actions.append(
                    Action(
                        kind="write",
                ...
                actions.append(Action(kind="read", path=_first(args, _PATH_KEYS), tool=name))
            elif basename in _EXEC_TOOLS or any(m in basename for m in _EXEC_MARKERS):
                actions.append(Action(kind="exec", command=_first(args, _COMMAND_KEYS), tool=name))
    return actions
