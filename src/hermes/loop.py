"""Loop Engineering core state machine for Hermes.
...
- Seven stop rules (check_stop_rules)
- Checkpoint / rewind

模块拆分（本文件曾是 2273 行的巨型文件）：
- ``loop_patterns``  — 静态定义层：LOOP_PATTERNS / STOP_RULES / 枚举 / 红线（无状态）
- ``loop``（本文件）— 状态机层：LoopState/LoopRound、meta.json 持久化、
                        init/record_round/checkpoint/rewind、stop rules 评估
- ``loop_audit``     — 审计层：audit_loop / audit_deliverables / knowledge_hygiene_scan
- ``loop_gepa``      — GEPA 接线层：evaluator 注入 + 终态触发自进化周期

为保持向后兼容（tests/ 与 CLI 大量 ``from hermes.loop import X``），
本文件从拆分模块 re-export 全部公共符号；``__all__`` 标注真实归属。
"""
...
# ── Re-exports（向后兼容，真实定义已拆分）────────────────────────────
from hermes.loop_patterns import (  # noqa: F401
    BUILDER_RED_LINES,
...
from hermes.loop_audit import (  # noqa: F401
    audit_deliverables,
    audit_loop,
    knowledge_hygiene_scan,
)
from hermes.loop_gepa import (  # noqa: F401
    GepaEvaluator,
