"""Append-only dispatch trajectory log + reconstruction invariant.

A control plane (agent orchestrator) dispatches sub-agents through gateways or
direct runtimes. This module records an append-only snapshot of every dispatch
request (and its result) so the exact input that was sent can be replayed and
audited later.
...
log-write bugs, on-disk tampering, and field drift (via contract tests).

An offline audit (:func:`verify_trajectory`) checks a trajectory file for
line integrity, sequence continuity, request/result pairing completeness, and
agent-file hash consistency.

All writes are guarded by an in-process lock plus a file-level lock (the
file-level lock only makes each line atomic; the in-process lock serializes
the in-memory sequence counter).

Extracted from the Hermes agent framework (hermes.trajectory, ADR-0017),
with the storage primitive inlined so the package has zero dependencies.
"""
...
from ._append import atomic_append_jsonl

logger = logging.getLogger("traj_verify")
...
def archive_trajectory(path: Path) -> Path | None:
    """Rename an existing trajectory file to ``<stem>.<cycle>.jsonl``.

    Used before starting a fresh cycle so events from different cycles do not
    interleave. Returns the archive path, or None if there was nothing to
    archive.
    """
    if not path.exists():
        return None
...
def verify_trajectory(path: Path) -> dict[str, Any]:
    ...
    for ev in events:
        ...
        if isinstance(request_seq, int):
            ...
            # A non-int request_seq (e.g. None) is a malformed result —
            # report it explicitly instead of silently dropping it.
            malformed_results.append(request_seq)
