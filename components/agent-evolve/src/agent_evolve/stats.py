"""Statistical significance for promotion decisions (split-run + Welch's t-test).

A variant must not be promoted on a single lucky run. Promotion requires
*min_repeats* independent evaluation runs per variant and a statistically
significant score difference (Welch's t-test, p < alpha).
...
Extracted from the Hermes agent framework (hermes.gepa_stats).
