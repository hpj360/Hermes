"""GEPA (Generate-Evaluate-Promote-Apply) cycle for agent self-evolution.

A minimal, dependency-free engine for evolving *agent definitions* (not just
prompts):

- Generate: caller supplies variant agent definitions, or an LLM generates
  candidate strategies behind a safety screen.
- Evaluate: each variant runs on a benchmark task via a caller-supplied
  function (the engine stays decoupled from any specific agent runtime).
- Promote: pick the winner by an explicit scoring function
  (success >> tokens >> rounds), statistically gated in split-run mode.
- Apply: persist every experiment (success or failure) for audit; never
  overwrite.

Design principles:

- Decoupled: evaluation is delegated to a caller-supplied callable, so the
  engine is testable in isolation and works with any agent runtime.
- Audit-first: every experiment is persisted. Self-evolution without an audit
  trail is dangerous (a bad promotion can silently regress the system).
- Conservative promotion: only successful variants get promoted. If all
  variants fail, no promotion happens — better no evolution than wrong
  evolution.
- Scoring is explicit and tunable, not magic.

Extracted from the Hermes agent framework (hermes.gepa, ADR series on
self-evolution), with the storage directory and LLM protocol decoupled.
"""
...
from .stats import mean, should_promote

logger = logging.getLogger("agent_evolve.evolve")
...
@dataclass
class Variant:
    """A candidate agent-definition variant for evaluation.

    The agent_file field points to an agent definition file (e.g., a
    builder.md with a different prompt strategy). The engine itself does not
    parse the file — it just passes the path to the evaluation function.
    """
...
class VariantResult:
    ...
    Fields map cleanly onto typical agent-loop metrics:
    - success: did the variant solve the benchmark task?
    - tokens_used: total tokens consumed (efficiency signal)
    - rounds_to_converge: how many iterations to all-green
...
@dataclass
class Experiment:
    """A complete evolution cycle: variants + benchmark + results + winner.
...
# Backwards/forwards-compatible alias: the engine was originally named GEPA.
GEPAExperiment = Experiment
...
class LlmJsonClient(Protocol):
    """Minimal protocol an LLM client must satisfy for variant generation.

    Any client with a ``chat_json(messages) -> dict`` method works, where
...
def _build_variant_generation_prompt(
    ...
    lines = [
        "You are designing agent-definition variants for a self-improvement cycle.",
...
def auto_generate_variants(
    ...
    *,
    ...
    ``<registry>/variants/``) and wrapped in a :class:`Variant`.
    ...
    out_dir = output_dir or (registry_dir() / "variants")
...
# so the engine stays decoupled from any specific agent runtime.
...
def run_cycle(
    ...
    """Run one evolution cycle: evaluate all variants on the benchmark, pick winner.

    Args:
        benchmark_task: The task description all variants must solve.
        variants: List of Variant objects (each points to an agent file).
    ...
    Returns:
        Experiment with results filled in and winner_id set (or None if
    ...
    if not variants:
        ...
        logger.warning("cycle %s: no variants provided", experiment.experiment_id)
        return experiment
    ...
    for variant in variants:
        try:
            ...
            logger.exception(
                "cycle %s: variant %s evaluation crashed",
    ...
    if successful:
        ...
        logger.warning(
            "cycle %s: no variant succeeded", experiment.experiment_id,
        )
...
run_gepa_cycle = run_cycle


# ── Split-run cycle (statistical significance) ──────────────────────
...
def run_split_run(
    ...
    *,
    ...
    """Run a statistically-gated split-run.
    ...
    see :mod:`agent_evolve.stats`). This is the gate that prevents a single
    lucky run from being promoted.

    Returns an :class:`Experiment` whose ``winner_id`` is set only when a
    ...
    # the experiment-wide error rate stays at the caller's alpha.
    ...
    for challenger in challengers:
        ...
        if should_promote(baseline_scores, c_scores, alpha=alpha_adj, min_repeats=min_repeats) and (
            winner is None or mean(c_scores) > mean(winner_scores)
        ):
            winner = challenger
...
# Backwards/forwards-compatible alias.
run_gepa_split_run = run_split_run
...
def registry_dir() -> Path:
    """Return the experiment registry directory.

    Resolution order:
    1. ``AGENT_EVOLVE_DIR`` environment variable (absolute path).
    2. ``<cwd>/.agent-evolve``

    The directory is created lazily on first save.
    """
    env = os.environ.get("AGENT_EVOLVE_DIR")
    if env:
        return Path(env)
    return Path.cwd() / ".agent-evolve"


def save_experiment(experiment: Experiment, *, registry: Path | None = None) -> Path:
    """Persist an experiment to ``<registry>/<experiment_id>.json``.

    Returns the path written. Creates the registry if missing. Audit trail:
    every experiment (success or failure) is persisted, never overwritten.
    """
    out_dir = registry if registry is not None else registry_dir()
    out_dir.mkdir(parents=True, exist_ok=True)
...
def load_experiment(experiment_id: str, *, registry: Path | None = None) -> Experiment | None:
    """Load an experiment by ID. Returns None if not found or unreadable."""
    out_dir = registry if registry is not None else registry_dir()
    ...
    try:
        ...
        logger.warning("Failed to load experiment %s", experiment_id)
        return None
    return Experiment.from_dict(data)


def list_experiments(*, registry: Path | None = None) -> list[Experiment]:
...
def get_latest_promotion(*, registry: Path | None = None) -> Experiment | None:
    ...
    """
    for experiment in list_experiments(registry=registry):
